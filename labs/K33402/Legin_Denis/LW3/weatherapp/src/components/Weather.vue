            ...
            <div class="container">
                ...
                <div class="container-temp">
                    <div class='mt-2 d-flex justify-content-center' v-if="info.main.temp">
                        <p class='head-text'>Температура: {{ info.main.temp }} °c</p>
                    </div>
                </div>
...
export default {
  ...
  methods: {
    ...
    getWeather () {
      axios
        .get('https://api.openweathermap.org/data/2.5/weather?q=' + this.city + '&appid=62f76307202d2bbb00f83a4de8ac7393&lang=ru&units=metric')
    ...
    async getLocation () {
      ...
      axios
        .get('https://api.openweathermap.org/data/2.5/weather?lat=' + this.location.coords.latitude + '&lon=' + this.location.coords.longitude + '&appid=62f76307202d2bbb00f83a4de8ac7393&lang=ru&units=metric')
        .then(response => (this.info = response.data))
    }
  }
}
